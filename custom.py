...
products2 = {
    ...
    'Аніме брєлок(Naruto)': {'price': 2.5, 'image': 'akaz.png'}
}
cart = {}
...
class LoginScreen(Screen):
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            Color(0.4, 0.0, 0.0, 1)  # Синій фон
            self.rect = Rectangle(size=self.size, pos=self.pos)
            self.bind(size=self._update_rect, pos=self._update_rect)

        layout = BoxLayout(orientation='vertical', padding=10, spacing=10)
        layout.add_widget(Image(source='pr.png'))
        layout.add_widget(Label(text='Ласкаво просимо до аніме магазину!', font_size='18sp', color=(1, 1, 1, 1)))
...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            Color(0.3, 0.0, 0.0, 1)  # Синій фон
            self.rect = Rectangle(size=self.size, pos=self.pos)
            self.bind(size=self._update_rect, pos=self._update_rect)

        layout = BoxLayout(orientation='vertical', padding=10, spacing=10)

        # Верхний контейнер с двома кнопками
        ...
        # Додаємо кнопку "2 сторинка" справа
...
class SecondScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Кнопка "Головна сторинка"
        back_btn = Button(
            text='Головна сторинка',
            size_hint=(None, None),
            size=('150dp', '40dp'),
            background_normal='',
            background_color=(0.7, 0.1, 0.1, 1),  # Червоний колір кнопки
        ...
        for product in products2:
            ...
            image = Image(source=products2[product]['image'], size_hint=(None, None), size=('100dp', '100dp'))

            info_layout = BoxLayout(orientation='vertical')
            description = Label(text=f"{product} - {products2[product]['price']}$", font_size='18sp', color=(1, 1, 1, 1))
            add_button = Button(
                text='Додати до корзини',
                size_hint=(1, None),
                height='40dp',
...
class CartScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.label = Label(text='Корзина порожня', font_size='18sp', color=(1, 1, 1, 1))
